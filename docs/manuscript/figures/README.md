...
Run the R figure scripts 30–37 from the project root, then run `build_figure_bundle.py` with a Python environment containing pypdf. The figure builders read the validated outputs and metadata. The package script checks that every PDF has one page and no PDF annotation objects; `figure_export_manifest.json` records dimensions and SHA256 checksums. The before-redesign snapshot and design review are in `reports/figure_redesign_2026-09-05` at the project root.
