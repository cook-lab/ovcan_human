#!/usr/bin/env python3
"""Build the Scientific Data v6 working manuscript from its Markdown source.

This script writes only to the new v6 manuscript directory.
"""
...
SOURCE = ROOT / "OvCAN_Scientific_Data_draft_v6.md"
OUTPUT = ROOT / "OvCAN_Scientific_Data_draft_v6.docx"
...
def add_document_metadata(doc: Document) -> None:
    ...
    props.comments = "Version 6; explicit TODO markers link to author_confirmation.md."
...
def add_figures(doc: Document) -> None:
    ...
    for number in range(1, 5):
        ...
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run()
        shape = run.add_picture(str(matches[0]), width=Inches(6.5 if number < 4 else 6.25))
