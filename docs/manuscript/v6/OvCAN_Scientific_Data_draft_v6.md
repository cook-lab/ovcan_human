...
RNA principal components showed separation among recorded histotype groups (Fig. 3a,b). For the first component, which explained 20.6% of expression variance, histotype accounted for a unique 42.4% of the variance in component scores, centre for 0.2%, and their shared component for 31.0%. After selecting one model per patient and refitting the variable-gene selection and principal components, the corresponding fractions were 52.0%, 0.3%, and 26.9% (Fig. 3d). All high-grade serous RNA models originated from one centre, so effects of culture or processing aligned with histotype cannot be separated completely from histotype-associated expression.

Among 25 prespecified marker checks, 15 were recovered, compared with a within-centre permutation mean of 8.46 (permutation P = 0.0101) (Fig. 3e,f). This aggregate assessment supports the presence of established expression patterns in the panel. Histotype groups with few independent patients remain limited for inference about their broader populations, and the TOV112D annotation requires particular care in comparisons involving the historical endometrioid group.
...
(a) Overview of newly generated RNA and protein data and reprocessed whole-exome data. (b) Assay coverage for 42 models, ordered by recorded histotype and patient of origin. Filled cells indicate available RNA, protein, candidate-variant, or copy-number data; 13 models have all three principal data types. (c) Model and independent-patient counts by histotype group. Historical group labels are retained as described in Table 1. HGS, high-grade serous; CC, clear cell; MC, mucinous; EC, endometrioid; MMMT, carcinosarcoma; SCCOHT, small cell carcinoma of the ovary, hypercalcaemic type; LGS, low-grade serous.

### Figure 2 Quality assessment of the RNA and protein records

(a) Gene detection and pseudoalignment across 31 RNA libraries. Symbols indicate contributing centre; open symbols indicate centre medians, and grey reference lines indicate overall medians. (b) Gene detection versus processed paired-end fragments, with a linear fit and 95% confidence interval. (c) Interquartile ranges of transcript and protein abundance across models for 8,035 paired genes. RNA values are log2(TPM + 1); protein values are supplied log2 normalised abundances. Boxes indicate quartiles and medians; whiskers extend to values within 1.5 times the interquartile range. (d) Protein-feature detection across tandem mass tag plexes. (e) Supplied and bridge-derived coefficients of variation by protein-abundance decile; shading indicates the interquartile range of supplied values. (f) Bland–Altman plots for four samples measured in adjacent plexes. The bridge pairs use different samples, and each point represents a protein feature measured in both members of the pair. Solid lines indicate mean differences; dashed lines indicate 95% limits of agreement, calculated as the mean difference ± 1.96 standard deviations.

### Figure 3 Expression patterns and agreement between assays

(a,b) RNA principal components for all 31 models, coloured by histotype or centre. (c) Distribution of per-gene Spearman correlations between RNA and protein profiles across 30 paired models, with at least 10 paired observations per gene. (d) Unadjusted partition of variance in the first principal-component scores into unique histotype, unique centre, and shared contributions, comparing all models with 28 patient representatives. Variable-gene selection and principal components were refitted in the representative subset. (e) Marker means across 28 patient representatives, scaled across the six expression-profiled histotype groups; solid and dashed boxes identify expected high and low expression, respectively. The purple outline identifies the historical endometrioid marker group. (f) Marker abundance across all 31 individual models. Coloured points mark the expected histotype, and grey points show other models. The 25 marker checks were assessed using 20,000 joint label permutations within centre. MKI67 is an additional proliferation control and is excluded from the recovery count. Labels refer to the recorded histotype groups.

### Figure 4 Genomic filtering and external expression matching

(a) Variant counts before and after caller-quality, population-frequency, and consequence filtering in 23 models. (b) Expression correlations of five models with 67 DepMap ovarian models; each namesake ranks first. (c) Candidate alterations in selected ovarian cancer genes, coloured by evidence tier. Patient-frequency bars use Tier 1 and Tier 2 calls and count each patient once if a qualifying call occurs in any sequenced subline. Empty cells indicate no retained candidate in the displayed genes; they do not establish a wild-type genotype. A zero frequency can occur for genes with Tier 3 calls only. (d) Arm-level gain and loss frequencies among 11 high-grade serous patients, counting an event once per patient if present in any of their models.
