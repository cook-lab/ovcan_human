...
The separate [September molecular extension](../reports/molecular_extension_2026-09-06/README.md) uses scripts 42–50 and preserves the current release/manuscript. Its module reports specify inputs, seeds, reference caches and dependencies. Script 50 replays the approved ClinVar query from saved responses offline unless `--fetch` is explicitly requested. It is not part of the canonical `run_all.sh` rebuild. Start new cluster analysis requests with [the reviewed execution plan](cluster/molecular_extension_2026-09-06/EXECUTION_PLAN.md). Script 51 validates the hash-preserved incoming cluster patch files and generates explicit execution manifests/regions without cluster access. Script 49 invokes script 51 when the recovered inventory is present; it can also be run separately to refresh the execution inputs.
