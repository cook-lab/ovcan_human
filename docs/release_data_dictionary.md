...
`wes_cnv_segments.csv` uses GRCh38 zero-based, half-open intervals (`chromosome`, `start`, `end`). These segments were reconstructed with CNVkit 0.9.10 CBS after removing antitarget rows from the archived bin ratios. The four positive antitarget bins overlap capture targets and carry technical ratio artifacts. Original target ratios and their reference filtering were retained; low-coverage and outlier filtering were applied during segmentation. `probes` counts bins used for segmentation; `weight` is the CNVkit weight summed over bins overlapping the segment, which can also include bins excluded from breakpoint estimation. `log2_raw` is the reconstructed segment ratio before project centring; `log2c` includes chromosome X in its centring baseline; `log2c_auto` uses the autosomal probe-weighted median and is preferred. These ratios do not supply absolute or allele-specific copy number. Segment spans bridge unmeasured gaps between targets.
